  ...
  <div :class="wrap_class">
    <table :class="config.table_class">
      <thead :class="config.head_class">
        <tr>
          <th v-show="config.show_increment">Increment</th>
          <th
            class="text-capitalize"
            v-for="(thead, index) in header" :key="thead.id" :class="thead.class" v-show="hide_col.indexOf(index) === -1"
          >
            <span
              v-if="
                config.checkbox &&
          ...
          <th
            v-show="
              config.hasOwnProperty('undo_row') && config.undo_row
            "
          ></th>
        </tr>
      </thead>
      ...
      <tbody :class="config.body_class">
        <tr
          ...
          :class="{
            'tr-active':
              context_menu_row && context_menu_row.id === row.id,
          }"
        ...
        >
          ...
          <td
            v-for="(item, k, c) in row"
            :key="`${k}--${c}`"
            :class="body[c] && body[c].class"
            v-show="hide_col.indexOf(c) === -1"
          >
            <div
              v-if="
                body[c] &&
                body[c].hasOwnProperty('method') &&
                has_ev(body[c].method) &&
                !toggle_edit_row
              "
            >
              ...
              <!-- Hover -->
              <div
                if="check_ev(body[c].method, 'hover')"
                @mouseover="
                  hover(
                    check_ev(body[c].method, 'hover'),
                    row,
                    r,
                    c,
                    k
              ...
              >
                <span v-if="v === r && h === c">
                  <slot
                    v-if="check_slot(body[c].method)"
                    :name="check_slot(body[c].method)"
                    v-bind="{ row, r, c, k }"
                  ></slot>

                  <span v-else
                    >Can't find SLOT in PROP:
                    "body.method.slot"</span
                  >
                </span>
              ...
              <span class="simple-table-undo">
                <button
                  v-show="show_undo_cell(row.id, r, c)"
                  class="btn btn-xs btn-info"
                  @click.prevent="set_data_row_undo(r)"
                >
        ...
        <tr v-else class="tr-active">
          <td :colspan="header.length + 1">
            <slot
              name="edit_row"
              :row="find_by_id(toggle_edit_row)"
            ></slot>
          </td>
        </tr>
      </tbody>
...
.tr-active {
  background-color: rgba(148, 149, 158, 0.38) !important;
}
.ctx-menu {
...
export default {
  ...
  props: {
    ...
    }, // header key (sort property), class
    body: {
      default () {
        return {}
      }
    }, // body event, class, slot
    tabledata: {
      default () {
        return {}
      }
    }, // body data
    contextmenu_edit_text: {
      type: String,
      default () {
        return 'Edit'
      }
    },
    contextmenu_remove_text: {
      type: String,
      default () {
        return 'Remove'
      }
    ...
    }, // vertical index want hide column
    wrap_class: {
      type: String,
      default () {
        return ''
    ...
    checkall: {
      get: function () {
        return this.tabledata
          ? this.checkbox.length === this.tabledata.length
          : false
      },
      set: function (value) {
        var checkbox = []
  ...
  methods: {
    ...
    sort_colum (item) {
      ...
      this.tabledata.sort((a, b) => {
        ...
        if (
          ...
          nameA = a[item.key].toLowerCase()
          nameB = b[item.key].toLowerCase()
        } else {
          nameA = a[item.key]
          nameB = b[item.key]
        }
    ...
    set_edit_row () {
      // context_menu_row when right click context menu onCtxOpen()
    ...
    show_undo_cell (id, r, c) {
      if (
        Object.prototype.hasOwnProperty.call(this.config, 'undo_cell') &&
        this.config.undo_cell === true
      ) {
        return (
          this.row_undo &&
          this.row_undo === id &&
    ...
    show_undo_row (id) {
      if (
        Object.prototype.hasOwnProperty.call(this.config, 'undo_row') &&
        this.config.undo_row === true
      ) {
        return (
          this.row_undo && this.row_undo === id && this.uv === false
        )
      } else { return false }
    },
    set_row_undo (row, v) {
    ...
    set_data_row_undo (index) {
      this.$set(this.tabledata, index, this.data_row_undo)
      this.$emit(
        'undo_row_emit',
        JSON.parse(JSON.stringify(this.data_row_undo))
      )
    ...
    remove_row () {
      ...
      if (this.checkbox.length > 0) {
        ...
        const index = this.tabledata.indexOf(this.context_menu_row)
        const confirmStr = `Remove row: ${
            index + 1 === 1
              ? '1 ST'
              : index + 1 === 2
              ? '2 ND'
